...
foreach ( $tf as $key => $val ) 
  if ( $val['term'] == $tm and $val['week'] == $wk ) 
    $tfq[] = $val;
shuffle($tfq);
...
foreach ( $hw as $key => $val ) 
  if ( $val['term'] == $tm and $val['week'] == $wk and $val['difficulty'] < 2 and $val['hw'] == 1 ) 
    $hwq[] = $val;
shuffle($hwq);
